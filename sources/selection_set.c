...
/*   Updated: 2018/11/30 17:59:21 by fsidler          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    set_selection_mode(t_selection *sel, t_handlemode mode, int set_colors)
{
    ...
    else if ((sel->mode & SCOP_TRANSLATE) || (sel->mode & SCOP_SCALE))
    {
        ...
        sel->rot[4] = mat4x4_trs(sel->offset[0], sel->quat[3], vec3_f(sel->scale[0].x));
        sel->rot[5] = mat4x4_trs(sel->offset[0], sel->quat[0], vec3_f(sel->scale[0].y));
        sel->rot[6] = mat4x4_trs(sel->offset[0], sel->quat[1], vec3_f(sel->scale[0].z));
    }
